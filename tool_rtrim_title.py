...
if __name__ == '__main__':
	import mdb
	db = mdb.AdoMdb()
	db.OpenDataBase('R:/#%20c5dba49d4810cff95595.mdb')
	#
	last_time = None
	max_fixtime_id = 1
	resultset = db.ExecuteQuery(u'SELECT log_id,log_title,log_posttime FROM blog_Article WHERE log_id>1', 500)
	for record in resultset:
		...
		if last_time is not None and (atricle_time-last_time).days<1:
			db.ExecuteUpdate(u"UPDATE blog_Article SET log_title='%s',log_intro='',log_posttime=log_posttime+%d WHERE log_id=%d" % (atricle_title, atricle_id-max_fixtime_id, atricle_id))
		else:
			db.ExecuteUpdate(u"UPDATE blog_Article SET log_title='%s',log_intro='' WHERE log_id=%d" % (atricle_title, atricle_id))
